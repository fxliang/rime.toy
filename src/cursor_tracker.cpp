...
CursorPosition CursorTracker::GetCursorPosition(HWND targetWindow) {
    ...
    if (cached_position_.valid && 
        cached_position_.targetWindow == targetWindow &&
        std::chrono::duration_cast<std::chrono::milliseconds>(
            now - cached_position_.timestamp).count() < cache_timeout_ms_) {
        cache_hit_count_++;
...
bool CursorTracker::TryGetMousePosition(POINT& pt) {
...
bool CursorTracker::IsPositionValid(const POINT& pt, HWND hwnd) {
    ...
    if (pt.x < -100 || pt.x > screenWidth + 100 ||
        pt.y < -100 || pt.y > screenHeight + 100) {
        return false;
    }
...
bool CursorTracker::ShouldUpdatePosition(const CursorPosition& newPos) {
    if (!cached_position_.valid) return true;
