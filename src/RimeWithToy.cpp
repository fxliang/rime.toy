...
void RimeWithToy::on_message(void *context_object, RimeSessionId session_id,
  ...
  if (!self || !message_type || !message_value)
    return;
  std::lock_guard<std::recursive_mutex> lock(self->m_message_mutex);
...
void RimeWithToy::UpdateUI(bool show) {
  ...
  std::lock_guard<std::recursive_mutex> lock(m_message_mutex);
  if (show) {
    if (status.composing) {
...
template <typename T, size_t N>
void _RimeParseStringOptWithFallback(RimeConfig *config, const char *key,
                                     T &value, const Array<T, N> &arr,
                                     const T &fallback) {
  char str_buff[256] = {0};
