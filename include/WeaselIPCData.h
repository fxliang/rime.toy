...
struct Status {
  Status()
      : type(SCHEMA), ascii_mode(false), composing(false), disabled(false),
        full_shape(false) {}
  void reset() {
    schema_name.clear();
    schema_id.clear();
...
template <typename Archive>
void serialize(Archive &ar, weasel::CandidateInfo &s,
...
template <typename Archive>
void serialize(Archive &ar, weasel::TextAttribute &s,
               const unsigned int version) {
  ar & s.range;
  ar & s.type;
}
